import { PencilRuler } from 'lucide-react';
import {
  forwardRef,
  useEffect,
  useImperativeHandle,
  useRef,
  useState,
} from 'react';
import { showToast } from '../../utils/helper';

interface Option {
  [key: string]: any;
}

interface MultiSelectProps {
  label: string;
  onChange?: (selected: string[]) => void;
  disabled?: boolean;
  value: string[];
  endPoints: (query: string) => Promise<any>;
  display: string;
  filterStr: string;
}

const MultiSelect = forwardRef<HTMLDivElement, MultiSelectProps>(
  (
    {
      label,
      value = [],
      onChange = () => {},
      disabled = false,
      endPoints,
      filterStr,
      display,
    },
    ref,
  ) => {
    const [options, setOptions] = useState<Option[]>([]);
    const [isOpen, setIsOpen] = useState(false);
    const wrapperRef = useRef<HTMLDivElement>(null);

    useImperativeHandle(ref, () => wrapperRef.current as HTMLDivElement);

    const handleFetchData = async () => {
      try {
        const res = await endPoints(filterStr);

        setOptions(res.data.data);
      } catch (error: any) {
        showToast('error', error?.message);
      }
    };

    useEffect(() => {
      if (endPoints!) {
        handleFetchData();
      }
    }, [endPoints, filterStr]);

    const toggleDropdown = () => {
      if (!disabled) setIsOpen((prev) => !prev);
    };

    const handleSelect = (optionValue: string) => {
      const newSelected = value.includes(optionValue)
        ? value.filter((val) => val !== optionValue)
        : [...value, optionValue];

      onChange(newSelected);
    };

    const removeOption = (val: string) => {
      const updated = value.filter((item) => item !== val);
      onChange(updated);
    };

    const selectedValues = options.filter((opt) => value.includes(opt.value));

    return (
      <div ref={wrapperRef} className="w-full">
        <label className="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
          {label}
        </label>

        <div className="relative z-20 inline-block w-full">
          <div className="relative flex flex-col items-center">
            <div onClick={toggleDropdown} className="w-full">
              <div
                className={`relative mb-2 flex h-11 rounded-lg border border-gray-300 py-1.5 pl-3 pr-3 shadow-theme-xs outline-none transition focus:border-primary-300 focus:shadow-focus-ring dark:border-gray-700 dark:bg-gray-900 dark:focus:border-primary-300`}
              >
                <span className="absolute text-gray-500 -translate-y-1/2 pointer-events-none left-3 top-1/2 dark:text-gray-400">
                  <PencilRuler className="w-4 h-4" />
                </span>
                <div className="flex flex-wrap flex-auto gap-2 pl-7">
                  {selectedValues.length > 0 ? (
                    selectedValues.map((opt) => (
                      <div
                        key={opt.value}
                        className="group flex items-center justify-center rounded-full border-[0.7px] border-transparent bg-gray-100 py-1 pl-2.5 pr-2 text-sm text-gray-800 hover:border-gray-200 dark:bg-gray-800 dark:text-white/90 dark:hover:border-gray-800"
                      >
                        <span className="flex-initial max-w-full">
                          {opt[display]}
                        </span>
                        <div className="flex flex-row-reverse flex-auto">
                          <div
                            onClick={() => removeOption(opt.value)}
                            className="pl-2 text-gray-500 cursor-pointer group-hover:text-gray-400 dark:text-gray-400"
                          >
                            <svg
                              className="fill-current"
                              width="14"
                              height="14"
                              viewBox="0 0 14 14"
                            >
                              <path
                                fillRule="evenodd"
                                clipRule="evenodd"
                                d="M3.40717 4.46881C3.11428 4.17591 3.11428 3.70104 3.40717 3.40815C3.70006 3.11525 4.17494 3.11525 4.46783 3.40815L6.99943 5.93975L9.53095 3.40822C9.82385 3.11533 10.2987 3.11533 10.5916 3.40822C10.8845 3.70112 10.8845 4.17599 10.5916 4.46888L8.06009 7.00041L10.5916 9.53193C10.8845 9.82482 10.8845 10.2997 10.5916 10.5926C10.2987 10.8855 9.82385 10.8855 9.53095 10.5926L6.99943 8.06107L4.46783 10.5927C4.17494 10.8856 3.70006 10.8856 3.40717 10.5927C3.11428 10.2998 3.11428 9.8249 3.40717 9.53201L5.93877 7.00041L3.40717 4.46881Z"
                              />
                            </svg>
                          </div>
                        </div>
                      </div>
                    ))
                  ) : (
                    <input
                      disabled={disabled}
                      placeholder="Select option"
                      readOnly
                      value="Select option"
                      className="w-full h-full pl-0 p-1 pr-2 text-sm bg-transparent border-0 outline-none appearance-none placeholder:text-gray-500 focus:border-0 focus:outline-none focus:ring-0 dark:placeholder:text-white"
                    />
                  )}
                </div>

                <div className="flex items-center py-1 pl-1 pr-1 w-7">
                  <button
                    type="button"
                    onClick={toggleDropdown}
                    className="w-5 h-5 text-gray-700 outline-none cursor-pointer focus:outline-none dark:text-gray-400"
                  >
                    <svg
                      className={`stroke-current ${isOpen ? 'rotate-180' : ''}`}
                      width="20"
                      height="20"
                      viewBox="0 0 20 20"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
                      <path
                        d="M4.79175 7.39551L10.0001 12.6038L15.2084 7.39551"
                        stroke="currentColor"
                        strokeWidth="1.5"
                        strokeLinecap="round"
                        strokeLinejoin="round"
                      />
                    </svg>
                  </button>
                </div>
              </div>
            </div>

            {isOpen && (
              <div
                className="absolute left-0 z-40 w-full overflow-y-auto bg-white rounded-lg shadow top-full max-h-select dark:bg-gray-900"
                onClick={(e) => e.stopPropagation()}
              >
                <div className="flex flex-col">
                  {options
                    .filter((opt) => !value.includes(opt.value))
                    .map((opt) => (
                      <div
                        key={opt.value}
                        className="hover:bg-primary-500/5 w-full cursor-pointer rounded-t border-b border-gray-200 dark:border-gray-800"
                        onClick={() => handleSelect(opt.value)}
                      >
                        <div className="relative flex w-full items-center p-2 pl-2">
                          <div className="mx-2 leading-6 text-gray-800 dark:text-white/90">
                            {opt[display]}
                          </div>
                        </div>
                      </div>
                    ))}
                </div>
              </div>
            )}
          </div>
        </div>
      </div>
    );
  },
);

MultiSelect.displayName = 'MultiSelect';
export default MultiSelect;
